...
*
******************************************************************************/
__STATIC_INLINE uint8_t Cy_BLE_GetNumOfActiveConn()
{
    uint32_t i;
    uint8_t connNum = 0u;
...
__STATIC_INLINE cy_stc_ble_conn_handle_t Cy_BLE_GetConnHandleByBdHandle(uint8_t bdHandle)
{
    ...
    {
        if((cy_ble_connHandle[i].bdHandle == bdHandle) &&
           (Cy_BLE_GetConnectionState(cy_ble_connHandle[i]) >= CY_BLE_CONN_STATE_CONNECTED))
        {
            connHandle = cy_ble_connHandle[i];
            break;
        }
    }
    if(connHandle.attId == CY_BLE_INVALID_CONN_HANDLE_VALUE)
    {
        /* HALT in debug mode */
        CY_ASSERT(0u);
    }
...
/** @} group_common_api_gap_functions */
