...
*
******************************************************************************/
void Cy_BLE_EventHandler(cy_en_ble_event_t eventCode,
                         void *eventParam)
{
    ...
    switch(eventCode)
    {
        ...
        case CY_BLE_EVT_TIMEOUT:
            Cy_BLE_TimeOutEventHandler((cy_stc_ble_timeout_param_t*)eventParam);
            #if (CY_BLE_GAP_ROLE_PERIPHERAL || CY_BLE_GAP_ROLE_CENTRAL)
            (void)Cy_BLE_InvokeServiceEventHandler((uint32_t)eventCode, (void*)eventParam);
            #endif /*(CY_BLE_GAP_ROLE_PERIPHERAL || CY_BLE_GAP_ROLE_CENTRAL)*/
            break;

        case CY_BLE_EVT_STACK_BUSY_STATUS:
            #if (CY_BLE_GAP_ROLE_PERIPHERAL || CY_BLE_GAP_ROLE_CENTRAL)
            cy_ble_busyStatus[Cy_BLE_GetConnHandleByBdHandle(((cy_stc_ble_l2cap_state_info_t*)eventParam)->bdHandle).attId] =
                ((cy_stc_ble_l2cap_state_info_t*)eventParam)->flowState;
            #endif /*(CY_BLE_GAP_ROLE_PERIPHERAL || CY_BLE_GAP_ROLE_CENTRAL)*/
            break;
        ...
        #if (CY_BLE_MODE_PROFILE)
            {
                cy_stc_ble_memory_request_t *memReq = (cy_stc_ble_memory_request_t*)eventParam;
        ...
        case CY_BLE_EVT_GAP_AUTH_COMPLETE:
        #if ((CY_BLE_GAP_ROLE_PERIPHERAL || CY_BLE_GAP_ROLE_CENTRAL) && (CY_BLE_BONDING_REQUIREMENT == CY_BLE_BONDING_YES))
            {
                ...
                connectedHandle = Cy_BLE_GetConnHandleByBdHandle(((cy_stc_ble_gap_auth_info_t*)eventParam)->bdHandle);
                cy_ble_peerBonding[connectedHandle.attId] = ((cy_stc_ble_gap_auth_info_t*)eventParam)->bonding;
            }
        #endif  /* CY_BLE_BONDING_REQUIREMENT == CY_BLE_BONDING_YES */
            break;

        case CY_BLE_EVT_GAP_CONNECTION_UPDATE_COMPLETE:
            Cy_BLE_GAP_ConnUpdateCompleteEventHandler((const cy_stc_ble_gap_conn_param_updated_in_controller_t*)eventParam);
            #if (CY_BLE_GAP_ROLE_PERIPHERAL || CY_BLE_GAP_ROLE_CENTRAL)
            (void)Cy_BLE_InvokeServiceEventHandler((uint32_t)eventCode, (void*)eventParam);
            #endif /*(CY_BLE_GAP_ROLE_PERIPHERAL || CY_BLE_GAP_ROLE_CENTRAL) */
            break;
        ...
        case CY_BLE_EVT_GAP_ENHANCE_CONN_COMPLETE:
        {
            ...
            connEnhanceParam = (cy_stc_ble_gap_enhance_conn_complete_param_t*)eventParam;
        ...
        case CY_BLE_EVT_L2CAP_CONN_PARAM_UPDATE_RSP:
            Cy_BLE_L2Cap_ConnParamUpdateRspEventHandler((cy_stc_ble_l2cap_conn_update_rsp_param_t*)eventParam);
        ...
        case CY_BLE_EVT_GATT_CONNECT_IND:
            ...
            if((Cy_BLE_GetConnectionState(*(cy_stc_ble_conn_handle_t*)eventParam) ==
                CY_BLE_CONN_STATE_CLIENT_DISCONNECTED_DISCOVERED) &&
               (((cy_stc_ble_conn_handle_t*)eventParam)->bdHandle ==
                cy_ble_connHandle[((cy_stc_ble_conn_handle_t*)eventParam)->attId].bdHandle))
            {   /* Set a discovered state */
                Cy_BLE_SetConnectionState(*(cy_stc_ble_conn_handle_t*)eventParam, CY_BLE_CONN_STATE_CLIENT_DISCOVERED);
            }
        ...
        #endif /* CY_BLE_GATT_ROLE_CLIENT */
            {
                ...
                #if (CY_BLE_GATT_ROLE_CLIENT)
                (void)Cy_BLE_GATTC_RemoveConnHandle(*(cy_stc_ble_conn_handle_t*)eventParam);
                #endif /* CY_BLE_GATT_ROLE_CLIENT */
                Cy_BLE_SetConnectionState(*(cy_stc_ble_conn_handle_t*)eventParam, CY_BLE_CONN_STATE_CONNECTED);
                cy_ble_connHandle[((cy_stc_ble_conn_handle_t*)eventParam)->attId] = *(cy_stc_ble_conn_handle_t*)eventParam;
            }

            cy_ble_busyStatus[((cy_stc_ble_conn_handle_t*)eventParam)->attId] = CY_BLE_STACK_STATE_FREE;

        #if ((CY_BLE_GATT_ROLE_SERVER) && (CY_BLE_GATT_DB_CCCD_COUNT != 0u) && \
            (CY_BLE_BONDING_REQUIREMENT == CY_BLE_BONDING_YES))
            /* Initialize the CCCD values in RAM when bonding is enabled */
            (void)memcpy((uint8_t*)cy_ble_attValuesCccdMultiple[((cy_stc_ble_conn_handle_t*)eventParam)->attId],
                         cy_ble_configPtr->flashStorage->attValuesCCCDFlashMemory[((cy_stc_ble_conn_handle_t*)eventParam)->
                                                                                   bdHandle],
        ...
        #if (CY_BLE_GATT_ROLE_CLIENT)
            if((Cy_BLE_GetConnectionState(*(cy_stc_ble_conn_handle_t*)eventParam) == CY_BLE_CONN_STATE_CLIENT_DISCOVERED) &&
               (CY_BLE_BONDING_REQUIREMENT == CY_BLE_BONDING_YES))
            {
                Cy_BLE_SetConnectionState(*(cy_stc_ble_conn_handle_t*)eventParam,
                                          CY_BLE_CONN_STATE_CLIENT_DISCONNECTED_DISCOVERED);
            }
            else
        #endif /* CY_BLE_GATT_ROLE_CLIENT */
            {
                Cy_BLE_SetConnectionState(*(cy_stc_ble_conn_handle_t*)eventParam, CY_BLE_CONN_STATE_DISCONNECTED);
            #if (CY_BLE_GATT_ROLE_CLIENT)
                (void)Cy_BLE_GATTC_RemoveConnHandle(*(cy_stc_ble_conn_handle_t*)eventParam);
            #endif /* CY_BLE_GATT_ROLE_CLIENT */
            }
        ...
        case CY_BLE_EVT_GATTS_XCNHG_MTU_REQ:
        {
            ...
            {
                mtuParam.mtu = ((cy_stc_ble_gatt_xchg_mtu_param_t*)eventParam)->mtu;
            }
            else
            {
        ...
        {
            cy_en_ble_gatt_err_code_t gattErr;
            gattErr = Cy_BLE_GATTS_WriteEventHandler((cy_stc_ble_gatts_write_cmd_req_param_t*)eventParam);
            if(gattErr == CY_BLE_GATT_ERR_NONE)
            {
                gattErr = Cy_BLE_InvokeServiceEventHandler((uint32_t)eventCode, (void*)eventParam);
            }
            Cy_BLE_SendWriteResponse((cy_stc_ble_gatts_write_cmd_req_param_t*)eventParam, gattErr);
        }
        break;
        ...
        case CY_BLE_EVT_GATTS_READ_CHAR_VAL_ACCESS_REQ:
            ...
            (void)Cy_BLE_GATTS_ReadAttributeValueCCCDReqHandler((cy_stc_ble_gatts_char_val_read_req_t*)eventParam);
        ...
        case CY_BLE_EVT_GATTC_READ_BY_GROUP_TYPE_RSP:
            Cy_BLE_ReadByGroupEventHandler((cy_stc_ble_gattc_read_by_grp_rsp_param_t*)eventParam);
            break;

        case CY_BLE_EVT_GATTC_READ_BY_TYPE_RSP:
            Cy_BLE_ReadByTypeEventHandler((cy_stc_ble_gattc_read_by_type_rsp_param_t*)eventParam);
            break;
        ...
        case CY_BLE_EVT_GATTC_LONG_PROCEDURE_END:
            Cy_BLE_LongProcedureEndEventHandler(((cy_stc_ble_gattc_long_procedure_end_param_t*)eventParam)->connHandle);
            break;

        case CY_BLE_EVT_GATTC_FIND_INFO_RSP:
            Cy_BLE_FindInfoEventHandler((cy_stc_ble_gattc_find_info_rsp_param_t*)eventParam);
            break;

        case CY_BLE_EVT_GATTC_ERROR_RSP:
            Cy_BLE_ErrorResponseEventHandler((cy_stc_ble_gatt_err_param_t*)eventParam);
            (void)Cy_BLE_InvokeServiceEventHandler((uint32_t)eventCode, (void*)eventParam);
            break;
        ...
        case CY_BLE_EVT_GATTC_HANDLE_VALUE_IND:
            Cy_BLE_GATTC_IndicationEventHandler((cy_stc_ble_gattc_handle_value_ind_param_t*)eventParam);
            (void)Cy_BLE_InvokeServiceEventHandler((uint32_t)eventCode, (void*)eventParam);
            ...
            if((cy_ble_eventHandlerFlag & CY_BLE_CALLBACK) == 0u)
            {
                ...
                confirmationParam.connHandle = ((cy_stc_ble_gattc_handle_value_ind_param_t*)eventParam)->connHandle;
    ...
    {
        cy_ble_eventHandlerFlag &= (uint8_t) ~CY_BLE_CALLBACK;
        Cy_BLE_ApplCallback((uint32_t)eventCode, eventParam);
    }
}
