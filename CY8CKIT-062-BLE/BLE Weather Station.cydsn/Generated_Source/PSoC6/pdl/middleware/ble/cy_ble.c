...
cy_en_ble_api_result_t Cy_BLE_Init(cy_stc_ble_config_t *config)
{
    ...
    else
    {
        ...
        (void)memset(cy_ble_peerBonding, (int8)CY_BLE_GAP_BONDING_NONE, sizeof(cy_ble_peerBonding));
...
cy_en_ble_api_result_t Cy_BLE_Disable(void)
{
    ...
    apiResult = CY_BLE_ERROR_INVALID_OPERATION;     /* To stop Controller call Cy_BLE_Disable() function on Host core */
#else
    ...
    switch(callbackParams->mode)
    {
        case (CY_SYSPM_CHECK_READY):
        ...
        #endif  /* (CY_BLE_MODE_PROFILE) */
            {
                ...
                {
                #if(!(CY_BLE_STACK_MODE_HOST_UART || CY_BLE_STACK_MODE_HOST_IPC))
                    if((blessState != CY_BLE_BLESS_STATE_EVENT_CLOSE) && (Cy_BLE_IsControllerActive() == CY_BLE_SUCCESS))
                    {
