...
typedef enum
{
    ...
    CY_BLE_SRVI_BAS,
    CY_BLE_SRVI_BAS_END = CY_BLE_SRVI_BAS + CY_BLE_BASC_SERVICE_COUNT - 1u,
    ...
    CY_BLE_SRVI_CUSTOMS,
    CY_BLE_SRVI_CUSTOMS_END = CY_BLE_SRVI_CUSTOMS + CY_BLE_CUSTOMC_SERVICE_COUNT - 1u,
    ...
    CY_BLE_SRVI_HIDS,
    CY_BLE_SRVI_HIDS_END = CY_BLE_SRVI_HIDS + CY_BLE_HIDSC_SERVICE_COUNT - 1u,
...
void Cy_BLE_EventHandler(cy_en_ble_event_t eventCode, void *eventParam);
...
extern uint8_t cy_ble_scanningIntervalType;
    ...
    #if defined CY_BLE_CTS_SERVER || defined CY_BLE_ESS_SERVER
extern cy_en_ble_gatt_err_code_t cy_ble_gattError;
    #endif /* defined CY_BLE_CTS_SERVER || defined CY_BLE_ESS_SERVER */
...
/**
 * \addtogroup group_ble_common_api_core_functions
 * @{
 */
...
/******************************************************************************
* Function Prototype: void Cy_BLE_SetGattError(cy_en_ble_gatt_err_code_t gattError);
***************************************************************************//**
*
*  Sets the GATT Error Code after the Authorization Code check
*  on the application layer on a CY_BLE_EVT_<service initials>_WRITE_CHAR event
*  for the Bond Management Control Point characteristic.
*
*  This API function is useful only within the registered service callback on a
*  CY_BLE_EVT_<service initials>_CHAR event for certain services:
*
*  BMS: Check the Authorization Code of the Bond Management Control Point characteristic.
*  CTS: To set a GATT error if one or several data fields were ignored
*       by the Server.
*  ESS: Used by the user to indicate the unsupported condition of ES Trigger Descriptor.
*  CGMS: Check CRC and the length of the characteristics.
*
*  cy_en_ble_gatt_err_code_t gattError: GATT Error Code, possible values are:
*  - CY_BLE_GATT_ERR_NONE - if the application layer decides the
*                             Authorization Code is correct for this OpCode.
*  - For the BMS:
*     - CY_BLE_GATT_ERR_OP_CODE_NOT_SUPPORTED - if the application layer decides the
*                             OpCode is not supported.
*     - CY_BLE_GATT_ERR_INSUFFICIENT_AUTHORIZATION - if the application layer decides the
*                             Authorization Code is not correct for this OpCode.
*  - For the CTS:
*     CY_BLE_GATT_ERR_CTS_DATA_FIELD_IGNORED - one or several data fields were
*                                             ignored.
*  - For the ESS:
*     - CY_BLE_GATT_ERR_CONDITION_NOT_SUPPORTED - to indicate that the requested
*                                                condition is not supported.
*  - For the CGMS:
*     - CY_BLE_GATT_ERR_MISSING_CRC - when the CRC is missed.
*     - CY_BLE_GATT_ERR_INVALID_CRC - when the CRC is incorrect.
*     - CY_BLE_GATT_ERR_INVALID_PDU - when the length of the attribute is incorrect.
*
******************************************************************************/
#define Cy_BLE_SetGattError(gattError)    (cy_ble_gattError = (gattError))
...
/** @} group_common_api_core_functions */
