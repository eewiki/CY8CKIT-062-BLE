...
cy_stc_ble_lnsc_t cy_ble_lnsc[CY_BLE_CONFIG_GATTC_COUNT];

/* Internal storage for last request handle to check the response */
static cy_ble_gatt_db_attr_handle_t cy_ble_lnscReqHandle[CY_BLE_CONFIG_GATTC_COUNT];
                                                          ...
                                                          uint8_t * attrValue)
{
    ...
    else if((charIndex >= CY_BLE_LNS_CHAR_COUNT) || (discIdx >= CY_BLE_CONFIG_GATTC_COUNT))
    {
        apiResult = CY_BLE_ERROR_INVALID_PARAMETER;
    }
...
static void Cy_BLE_LNSC_NotificationEventHandler(cy_stc_ble_gattc_handle_value_ntf_param_t *eventParam)
{
    ...
    if((discIdx < CY_BLE_CONFIG_GATTC_COUNT) && (Cy_BLE_LNS_ApplCallback != NULL))
    {
