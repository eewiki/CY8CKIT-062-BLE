...
*
* \note
* The following calculations are implemented, generally in floating point:<br>
*   freqMHz = freq / 1,000,000<br>
*   max amplitude Vpp = 1,000 * sqrt(drivelevel / 2 / esr) / 3.14 / freqMHz / cLoad<br>
*   gm_min mA/V = 5 * 4 * 3.14 * 3.14 * freqMhz^2 * cLoad^2 * 4 * esr / 1,000,000,000<br>
*   # amplifier sections = INT(gm_min / 4.5)<br>
*   As a result of the above calculations, max amplitude must be ≥ 0.5, and
*   # of amplifier sections must be ≤ 3, otherwise this function returns with a parameter error.<br>
*   atrim = if max amplitude < 0.5 then error<br>
*           else 2 * the following:<br>
*                    max amplitude < 0.6: 0<br>
*                    max amplitude < 0.7: 1<br>
*                    max amplitude < 0.8: 2<br>
*                    max amplitude < 0.9: 3<br>
*                    max amplitude < 1.15: 5<br>
*                    max amplitude < 1.275: 6<br>
*                    max amplitude >= 1.275: 7<br>
*   wdtrim = if max amplitude < 0.5 then error<br>
*            else 2 * the following:<br>
*                     max amplitude < 1.2: INT(5 * max amplitude) - 2<br>
*                     max amplitude >= 1.2: 3<br>
*   gtrim = if # amplifier sections > 3 then error<br>
*           else the following:<br>
*                # amplifier sections > 1: # amplifier sections<br>
*                # amplifier sections = 1: 0<br>
*                # amplifier sections < 1: 1<br>
*   rtrim = if gtrim = error then error<br>
*           else the following:<br>
*                freqMHz > 26.8: 0<br>
*                freqMHz > 23.33: 1<br>
*                freqMHz > 16.5: 2<br>
*                freqMHz <= 16.5: 3<br>
*   ftrim = if atrim = error then error<br>
*           else INT(atrim / 2)<br>
*
*******************************************************************************/
...
cy_en_sysclk_status_t Cy_SysClk_FllConfigure(uint32_t inputFreq, uint32_t outputFreq, cy_en_fll_pll_output_mode_t outputMode)
{
    ...
    if (returnStatus == CY_SYSCLK_SUCCESS) /* no errors */
    {
        ...
        if (outputMode != CY_SYSCLK_FLLPLL_OUTPUT_INPUT)
        {
            ...
            /* 1. Determine if output division by 2 is required. */
            ...
            /* 4. Compute the FLL reference divider value.
                  Formula is refDiv = ROUNDUP((1 / (trimStep[ccoRange] / 2)) * (inputFreq / ccoFreq)) */
                config.refDiv = (uint16_t)ceilf((float32_t)inputFreq /
                                                ((trimSteps[config.ccoRange] / 2.0f) * (float32_t)ccoFreq));
            ...
            /* 8. Compute the CCO_FREQ bits in CLK_FLL_CONFIG4 register. */
                config.cco_Freq = (uint16_t)
                    (floor((sqrt((((float32_t)ccoFreq / fMargin[config.ccoRange]) * 2.0f) - 1.0f) - 1.0f) /
                            trimSteps[config.ccoRange]) - 2.0f);
            }
            /* 9. Compute the settling count, using a 1-usec settling time. */
            config.settlingCount = (uint16_t)CY_SYSCLK_DIV_ROUNDUP(inputFreq, 1000000ul);
...
/* nominal trim step sizes */
#define CY_SYSCLK_PILO_COARSE_TRIM_STEP 1000ul
#define CY_SYSCLK_PILO_FINE_TRIM_STEP      5ul
/** \endcond */
...
int32_t Cy_SysClk_PiloTrim(uint32_t piloFreq)
{
    ...
    /* Do nothing if piloFreq is already within one fine trim step from the target */
    uint32_t diff = (uint32_t)abs((int32_t)piloFreq - (int32_t)CY_SYSCLK_PILO_TARGET_FREQ);
    if (diff >= CY_SYSCLK_PILO_FINE_TRIM_STEP)
    {
        /* decide whether to use coarse or fine trim */
        if (diff >= CY_SYSCLK_PILO_COARSE_TRIM_STEP)
        { /* coarse trim */
            curTrim = _FLD2VAL(SRSS_CLK_TRIM_PILO_CTL_PILO_CFREQ, SRSS->CLK_TRIM_PILO_CTL);
            if (piloFreq > CY_SYSCLK_PILO_TARGET_FREQ)
            { /* piloFreq too high. Increase the coarse trim value; yes, this is correct. */
                newTrim = curTrim + CY_SYSCLK_DIV_ROUND(piloFreq - CY_SYSCLK_PILO_TARGET_FREQ,
                                                        CY_SYSCLK_PILO_COARSE_TRIM_STEP);
                if (newTrim >= SRSS_CLK_TRIM_PILO_CTL_PILO_CFREQ_Msk) /* limit overflow */
                {
                    newTrim = SRSS_CLK_TRIM_PILO_CTL_PILO_CFREQ_Msk;
                }
            }
            else
            { /* piloFreq too low. Decrease the coarse trim value; yes, this is correct. */
                newTrim = curTrim - CY_SYSCLK_DIV_ROUND(CY_SYSCLK_PILO_TARGET_FREQ - piloFreq,
                                                        CY_SYSCLK_PILO_COARSE_TRIM_STEP);
            ...
            /* Update the coarse trim value */
            CY_SYSCLK_CLR_SET(SRSS->CLK_TRIM_PILO_CTL, SRSS_CLK_TRIM_PILO_CTL_PILO_CFREQ, newTrim);
        }
        else
        { /* fine trim */
            curTrim = _FLD2VAL(SRSS_CLK_PILO_CONFIG_PILO_FFREQ, SRSS->CLK_PILO_CONFIG);
            if (piloFreq > CY_SYSCLK_PILO_TARGET_FREQ)
            { /* piloFreq too high. Decrease the trim value. */
                newTrim = curTrim - CY_SYSCLK_DIV_ROUND(piloFreq - CY_SYSCLK_PILO_TARGET_FREQ,
                                                        CY_SYSCLK_PILO_FINE_TRIM_STEP);
                if ((int32_t)newTrim < 0) /* limit underflow */
                {
                    newTrim = 0;
                }
            ...
            CY_SYSCLK_CLR_SET(SRSS->CLK_PILO_CONFIG, SRSS_CLK_PILO_CONFIG_PILO_FFREQ, newTrim);
        }
    }
...
* it must be the last callback function that is registered.
*
        ...
        { /* Indicating that we can go into DeepSleep. Before doing so ... */
            ...
            for (fllpll = 0ul; fllpll < (SRSS_NUM_PLL + 1ul); fllpll++)
            {
                ...
                if (0ul != ((fllpll == 0ul) ? (_FLD2VAL(SRSS_CLK_FLL_CONFIG_FLL_ENABLE, SRSS->CLK_FLL_CONFIG)) : 
                                              (_FLD2VAL(SRSS_CLK_PLL_CONFIG_ENABLE, SRSS->CLK_PLL_CONFIG[fllpll]))))
                {
        ...
        if (changedSourcePaths != 0u)
        {
            ...
            else
            {
                ...
                for (fllpll = 0ul; fllpll < (SRSS_NUM_PLL + 1ul); fllpll++)
                {
                    /* If FLL or PLL is enabled, */
                    if (0ul !=
                        ((fllpll == 0ul) ? (_FLD2VAL(SRSS_CLK_FLL_CONFIG_FLL_ENABLE, SRSS->CLK_FLL_CONFIG)) : 
                                           (_FLD2VAL(SRSS_CLK_PLL_CONFIG_ENABLE, SRSS->CLK_PLL_CONFIG[fllpll]))))
                    {
