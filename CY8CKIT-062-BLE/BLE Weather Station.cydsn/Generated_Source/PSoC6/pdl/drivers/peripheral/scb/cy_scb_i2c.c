...
uint32_t Cy_SCB_I2C_SetDataRate(CySCB_Type *base, uint32_t dataRateHz, uint32_t scbClockHz)
{
    ...
    else
    {
        if ((scbClockHz > 0U) && (dataRateHz > 0U))
        {
            ...
            /* Set fitler configuration based on actual data rate */
...
* See \ref group_scb_i2c_macros_direction for the set of constants.
*
* \param timeoutMs
...
*
*******************************************************************************/
cy_en_scb_i2c_status_t Cy_SCB_I2C_MasterWriteByte(CySCB_Type *base, uint8_t theByte,
                                    uint32_t timeoutMs,
                                    cy_stc_scb_i2c_context_t *context)
{
    cy_en_scb_i2c_status_t retStatus = CY_SCB_I2C_MASTER_NOT_READY;
