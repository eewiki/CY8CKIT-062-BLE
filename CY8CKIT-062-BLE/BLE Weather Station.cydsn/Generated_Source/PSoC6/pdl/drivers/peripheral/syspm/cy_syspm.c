        ...
        if(0u != _FLD2VAL(SRSS_PWR_CTL_LPM_READY, SRSS->PWR_CTL))
        {
            ...
            */
            if(retVal == CY_SYSPM_SUCCESS)
            {
                ...
                /* Call the registered callback functions with the 
                * CY_SYSPM_BEFORE_TRANSITION parameter. The return value is 
                * ignored.
                */
                interruptState = Cy_SysLib_EnterCriticalSection();
                if(0u != currentRegisteredCallbacksNumber)
...
bool Cy_SysPm_RegisterCallback(cy_stc_syspm_callback_t* handler)
{
    ...
    (handler->callback != NULL))
    {
        ...
        {
            /* Check if there was the required callback already registered */
            while(regCallbackReturnValue && (currentCallback->nextItm != NULL))
            {
                if((currentCallback == handler) && 
                (currentCallback->callbackParams->base == handler->callbackParams->base))
                {
                    /* The condition to switch off from the checking loop and return 
                    * false as a requested callback is already registered
                    */
            ...
            /* Check the last element of the callback list */
            if((currentCallback == handler) && 
            (currentCallback->callbackParams->base == handler->callbackParams->base))
            {
                /* The condition to switch off from checking and return false */
                regCallbackReturnValue = false;
            }
            ...
            if(false != regCallbackReturnValue)
            {
                ...
                handler->prevItm = currentCallback;
                handler->nextItm = NULL;
...
bool Cy_SysPm_UnregisterCallback(cy_stc_syspm_callback_t* handler)
{
    ...
    if(0u != currentRegisteredCallbacksNumber)
    {
        ...
        (callbackRoot->callbackParams->base == handler->callbackParams->base))
        {
            /* Unregister the requested callback as it was first 
            *  registered callback
            */
        ...
        {
            /* Searching the callback by the handler */
            if(currentCallback->nextItm == handler)
            {
                nextCallback = currentCallback->nextItm;

                /* Check founded callback address */
                if((nextCallback->callbackParams->base == handler->callbackParams->base) && 
                (currentCallback->nextItm == handler))
                {
                    /* Update next element of the current list element */
                    currentCallback->nextItm = handler->nextItm;
                    
                    /* Update previous element of the next list element */
                    if(handler->nextItm != NULL)
                    {
                        nextCallback = nextCallback->nextItm;
                        nextCallback->prevItm = handler->prevItm;
                    }
...
****************************************************************************//**
*
* The internal function for setting a delay after switching the device from
* the deep sleep power mode.
*
...
static void Cy_SysPm_SetDelayAfterDeepSleep(void)
{
    ...
    {
        while((clkPath < CY_SYSPM_PWR_MAX_PLL_NUMBER) && (waitClocks != CY_SYSPM_PWR_TRIM_WAKE_CNTS))
        {
...
* when the voltage for the core regulators is less than 0.99 V. 
* This bit should be set if the output voltage for the core regulators is 
* higher than 0.99 V.
*
*******************************************************************************/
static void Cy_SysPm_SetVoltageBitForFlash(bool setBit)
