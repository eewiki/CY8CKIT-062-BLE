...
/** Class 1 - Highest class, safety critical functions which rely on parameters that could be
  * changed between different PSoC devices.
  */
#define CY_ASSERT_CLASS_1           1

/** Class 2 - Functions that have fixed limits such as counter periods (16-bits/32-bits etc.). */
#define CY_ASSERT_CLASS_2           2

/** Class 3 - Functions that accept enums as constant parameters. */
#define CY_ASSERT_CLASS_3           3

#ifndef CY_ASSERT_LEVEL
    /** User definable assert level from compiler command line argument (similarly to DEBUG / NDEBUG) */
    ...
    #define CY_ASSERT_L2(x)         do{} while(0u)    /**< Assert Level 2 */
    #define CY_ASSERT_L3(x)         do{} while(0u)    /**< Assert Level 3 */
