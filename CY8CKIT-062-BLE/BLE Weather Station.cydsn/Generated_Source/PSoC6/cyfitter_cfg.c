...
static void ClockInit(void)
{
	...
	Cy_SysClk_WcoEnable();
	{
		const cy_stc_wco_csv_config_t wcoCsv = 
		{
			.SupervisorClock =     CY_SYSCLK_WCO_CSV_SUPERVISOR_ILO,
			.enableLossDetection = 0,
			.LossWindow =          CY_SYSCLK_CSV_LOSS_4_CYCLES,
			.LossAction =          CY_SYSCLK_CSV_ERROR_IGNORE,
		};
		Cy_SysClk_WcoConfigureCsv(&wcoCsv);
	}
